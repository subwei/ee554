...
namespace test {
    ...
    public class BuildingVRMLNode : Node {
        ...
        /*
        public void castShawdow(float[] lp)
        {
            int i, j, jj;
            int p1, p2;
            Vertex v1, v2;
            double side;
            for (i = 0; i < PlaneEqs.Count; i++)
            {
                ArrayList plane = (ArrayList)PlaneEqs[i];
                side = (double)plane[0] * lp[0] + (double)plane[1] * lp[1] + (double)plane[2] * lp[2] + (double)plane[3] * lp[3];
                if (side > 0) plane.Add(true);
                else plane.Add(false);
            }
            ...
            for (i = 0; i < PlaneEqs.Count; i++)
            {
                ...
                if ((bool)plane[4])
                {
                    ...
                    {
                        //ArrayList Neighbors = (ArrayList)Connectivity[i];
                        //int neighborindex = (int)Neighbors[j];
                        //ArrayList plane2 = (ArrayList)PlaneEqs[neighborindex];
                        //if (!(bool)plane2[4])
                        {
                            p1 = (int)face[j];
                            jj = (j + 1) % face.Count;
                            ...
                            double x = (((Vertex)vertices[p1]).GetX() - lp[0]) * 100;
                            double y = (((Vertex)vertices[p1]).GetY() - lp[1]) * 100;
                            double z = (((Vertex)vertices[p1]).GetZ() - lp[2]) * 100;
                            v1 = new Vertex(x, y, z + 10);

                            x = (((Vertex)vertices[p2]).GetX() - lp[0]) * 100;
                            y = (((Vertex)vertices[p2]).GetY() - lp[1]) * 100;
                            z = (((Vertex)vertices[p2]).GetZ() - lp[2]) * 100;
                            v2 = new Vertex(x, y, z + 10);

                            //draw the polygon
                            Gl.glBegin(Gl.GL_TRIANGLE_STRIP);
                            Gl.glVertex3d(((Vertex)vertices[p1]).GetX(),
                                        ((Vertex)vertices[p1]).GetY(),
                                        ((Vertex)vertices[p1]).GetZ());
                            Gl.glVertex3d(((Vertex)vertices[p1]).GetX() + v1.GetX(),
                                        ((Vertex)vertices[p1]).GetY() + v1.GetY(),
                                        ((Vertex)vertices[p1]).GetZ() + v1.GetZ());

                            Gl.glVertex3d(((Vertex)vertices[p2]).GetX(),
                                        ((Vertex)vertices[p2]).GetY(),
                                        ((Vertex)vertices[p2]).GetZ());
                            Gl.glVertex3d(((Vertex)vertices[p2]).GetX() + v2.GetX(),
                                        ((Vertex)vertices[p2]).GetY() + v2.GetY(),
                                        ((Vertex)vertices[p2]).GetZ() + v2.GetZ());
                            Gl.glEnd();
                        }
                    }
                }
